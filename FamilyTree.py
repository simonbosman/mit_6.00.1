...
class Family(object):
    ...
    def get_parents(self, cousin, parents):
        if cousin == self.root:
            return parents
        else:
            parents.append(cousin.get_parent())
            return self.get_parents(cousin.get_parent(), parents)
    ...
    def cousin(self, a, b):
        ...
        cousin_node_a = self.names_to_nodes[a] 
        cousin_node_b = self.names_to_nodes[b]
        
        if cousin_node_a == cousin_node_b:
            return (-1, 0)
        
        cousin_node_a_ancestors = self.get_parents(cousin_node_a, [])
        cousin_node_b_ancestors = self.get_parents(cousin_node_b, [])
            
        if len(cousin_node_a_ancestors) < len(cousin_node_b_ancestors):
            if cousin_node_a in cousin_node_b_ancestors:
               return (-1, len(cousin_node_b_ancestors) - len(cousin_node_a_ancestors)) 
        
        if len(cousin_node_b_ancestors) < len(cousin_node_a_ancestors):
            if cousin_node_b in cousin_node_a_ancestors:
                return (-1,  len(cousin_node_a_ancestors) - len(cousin_node_b_ancestors)) 
        
                
        for ancestor in cousin_node_a_ancestors:
            if ancestor in cousin_node_b_ancestors:
                min_d = min([cousin_node_b_ancestors.index(ancestor), cousin_node_a_ancestors.index(ancestor)])
                delta_d =  max([cousin_node_b_ancestors.index(ancestor), cousin_node_a_ancestors.index(ancestor)]) 
                return (min_d, delta_d - min_d)
                break;    
...
#t, r = f.cousin("q", "q")
#print "'q' is a", words[t],"cousin", r, "removed from 'q'"

t, r = f.cousin("c", "g")
print "'c' is a", words[t],"cousin", r, "removed from 'g'"

t, r = f.cousin("o", "c")
print "'o' is a", words[t],"cousin", r, "removed from 'c'"
